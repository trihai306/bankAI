...
ipcMain.handle('tts:upload-ref', async (event, { audioData, filename }) => {
    try {
        const refPath = path.join(REF_AUDIO_DIR, filename || `ref_${Date.now()}.wav`);
...
ipcMain.handle('tts:transcribe-audio', async (event, audioPath) => {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            const transcribe = spawn(pythonPath, [scriptPath, audioPath, 'medium']);
            let stdout = '';
            let stderr = '';
            ...
            transcribe.on('close', (code) => {
                ...
                if (code === 0) {
                    ...
                    resolve({ success: false, error: `Transcription failed with code ${code}` });
                }
            });
